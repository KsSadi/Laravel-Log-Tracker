        ...
        <div class="content-card">
            ...
            <div class="search-bar">
                <div style="position: relative;">
                    ...
                    <input type="text" class="search-bar-input log-search" placeholder="Search through ALL log messages, timestamps, and more..." id="mainSearchInput">
                </div>
            </div>

            <!-- Pagination Controls -->
            <div class="pagination-container" id="topPagination">
                <div class="pagination-info" id="paginationInfo">
                    <span>Showing <span id="currentFrom">1</span> to <span id="currentTo">{{ min($perPage, count($entries)) }}</span> of <span id="totalFiltered">{{ count($entries) }}</span> entries</span>
                </div>
            ...
            <div id="bottomPagination" class="pagination-container" style="display: none;">
...
@push('scripts')
    <script>
        ...
        function filterLogs() {
            ...
            const mainSearchTerm = document.getElementById('mainSearchInput').value.toLowerCase();
            const topSearchInput = document.getElementById('topSearchInput');
            const topSearchTerm = topSearchInput ? topSearchInput.value.toLowerCase() : '';
            const searchTerm = mainSearchTerm || topSearchTerm;
            
            // Sync search inputs
            if (topSearchInput && mainSearchTerm !== topSearchTerm) {
                if (mainSearchTerm) {
                    topSearchInput.value = mainSearchTerm;
                } else {
                    document.getElementById('mainSearchInput').value = topSearchTerm;
                }
            }
        ...
        function updatePagination() {
            ...
            const topInfo = document.getElementById('topPaginationInfo');
            ...
            if (totalPages > 1) {
                if (topPagination) {
                    ...
                    generatePaginationButtons('topPaginationList', totalPages);
                }
                if (bottomPagination) {
        ...
        function formatStackTrace(index) {
            ...
            stackContent = stackContent
                ...
                .replace(/(#\d+)/g, '<span class="stack-frame">$1</span>')
                // Highlight file paths
                .replace(/([A-Za-z]:\\[^:\s]+\.[a-zA-Z]+)(\(\d+\))?/g, '<span class="stack-file">$1</span><span class="stack-line">$2</span>')
                // Highlight Unix paths
                .replace(/(\/[^:\s]+\.[a-zA-Z]+)(\(\d+\))?/g, '<span class="stack-file">$1</span><span class="stack-line">$2</span>')
